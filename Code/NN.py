...
result = Model.predict(flat_pixels)
p_vals = Model.predict_proba(flat_pixels)
...
outRaster = r"D:\final project\data2\output\NN.tif"
out_prob = r"D:\final project\data2\output\NN_Probability_Map.tif"
...
createGeotiff(outRaster, classification, geo_transform, projection, gdal.GDT_UInt16, bcount)
createGeotiff(out_prob, prob_array, geo_transform, projection, gdal.GDT_Float32, b_count)

#Accuracy check up.......................................................................
predicted_train_labels = Model.predict(trainingData)
train_lbl_cnt = (np.unique(trainingLabels)).size

predicted_test_labels = Model.predict(testingData)
...
df.to_csv(r'D:\final project\data2\output\NN_CM_TRAIN.csv', index=False)#CM==confusion_matrix

df = pd.DataFrame(check_accuracy(testingLabels, predicted_test_labels, test_lbl_cnt))
df.to_csv(r'D:\final project\data2\output\NN_CM_TEST.csv', index=False)

NN_accuracy = sm.accuracy_score(predicted_test_labels, testingLabels)
print('NN accuracy is ',sm.accuracy_score(predicted_test_labels, testingLabels))
...
kappa_score_test = cohen_kappa_score(testingLabels, predicted_test_labels)
print('kappa value testing: ', kappa_score_test)

print("Classified Tiff Image created!")
img = plt.imread(r"D:\final project\data2\output\NN.tif")
plt.imshow(img)
